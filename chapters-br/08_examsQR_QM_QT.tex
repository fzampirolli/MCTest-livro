...
\begin{itemize}
  \item Se o teste for do tipo \textit{Classical Testing Teory} (CTT), o parâmetro $b$ é calculado como a diferença entre 1 e a fração ponderada de respostas corretas em relação à quantidade total de questões corrigidas. Neste caso, esse valor de $b$ foi normalizado entre 0 e 10;
...
Nos tipos CTT e SAT, calcule a habilidade do estudante multiplicando elemento a elemento as listas \verb|b_vector| e \verb|u_vector|. Ao realizar essa operação e calcular a média os resultados, obtém-se um valor que representa a habilidade do estudante com base em suas respostas e nas dificuldades das questões.


Para uma boa calibração das questões, os tipos CTT e CAT exigem que cada questão tenha sido respondida por um número mínimo de participantes anteriormente. Por exemplo, o CAT, que se baseia na Teoria de Resposta ao Item (TRI), requer um mínimo de 1.000 participantes no modelo 3PLM \cite{min2021systematic}. Se a questão ainda não tiver sido respondida, a Taxonomia de Bloom é utilizada para classificá-la.


\begin{mybox}{corCopia}{\textbf{Atenção:\\\vspace{-3mm}\hrule\vspace{3mm}}}
  Para que o método adaptativo funcione corretamente:
\begin{enumerate}
\item Atribuir a Taxonomia de Bloom a cada questão. Essa atribuição é considerada uma condição inicial para o cálculo dos níveis de dificuldade das questões, tanto no método CTT quanto no CAT, antes de corrigir qualquer questão;
\item Certificar-se de que todos os exames anteriores tenham sido corrigidos e que as estatísticas de desempenho, tanto para o método CTT quanto para o CAT, tenham sido atualizadas para cada questão. Isso pode ser feito por meio da digitalização dos exames e do uso do botão ``Upload-PDF'' para realizar as correções;
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.9\textwidth]{cap08_figexameAdapterTypeQM.png}
...
O primeiro arquivo \verb|*_adaptive_test_variations.csv| possui na primeira coluna as 20 variações do exame, na segunda coluna a chave das variações no BD e na terceira coluna a média dos níveis da taxonomia de Bloom para as questões sorteadas para cada variação, em ordem crescente. Observam-se as questões com variações 4, 9, 12, 17 e 19 com o mesmo nível de dificuldade média de -0.333. Além disso, as variações 2, 5, 14 e 18 com dificuldade média 2. As colunas seguintes são para calibrar o parâmetros a e b da TRI de cada questão.

O primeiro arquivo \verb|*_adaptive_test_variations.csv| contém, na primeira coluna, as 20 variações do exame, na segunda coluna a chave das variações no banco de dados e, na terceira coluna, a média dos níveis da taxonomia de Bloom para as questões sorteadas para cada variação, em ordem crescente. Observam-se as questões com variações 4, 9, 12, 17 e 19 com o mesmo nível de dificuldade média de -0.333. A coluna seguinte apresenta o desvio padrão (STD). Além disso, as variações 2, 5, 14 e 18 têm dificuldade média de 2. As colunas seguintes são calibradas nos parâmetros $a$ e $b$ da TRI de cada questão ao corrigir um exame, fazendo \textit{upload} dos exames digitalizados.


\begin{myboxCode}{corCSV}{\textbf{Arquivo \texttt{*\_adaptive\_test\_variations}}}\vspace{3mm}
\hrule
\begin{verbatim}
Variation VariationID MeanAbilities STD   a1 a2 a3 b1 b2 b3
 4        61096       -0.333        1.247  0  0  0  0  1 -2
 9        61101       -0.333        1.247  0  0  0 -2  0  1
12        61104       -0.333        1.700  0  0  0 -2 -1  2
17        61109       -0.333        1.247  0  0  0  0  1 -2
19        61111       -0.333        1.700  0  0  0  2 -2 -1
 6        61098        0.000        2.160  0  0  0  3 -1 -2
15        61107        0.000        2.160  0  0  0 -2 -1  3
 1        61093        0.333        1.247  0  0  0  2 -1  0
 3        61095        0.667        1.700  0  0  0 -1  0  3
11        61103        0.667        2.055  0  0  0 -2  1  3
13        61105        0.667        2.055  0  0  0  1 -2  3
16        61108        0.667        1.700  0  0  0  3  0 -1
 7        61099        1.000        1.633  0  0  0  1 -1  3
10        61102        1.000        1.633  0  0  0 -1  1  3
 8        61100        1.333        1.247  0  0  0  0  1  3
20        61112        1.333        1.700  0  0  0  2 -1  3
 2        61094        2.000        0.816  0  0  0  2  3  1
 5        61097        2.000        0.816  0  0  0  3  2  1
14        61106        2.000        0.816  0  0  0  2  3  1
18        61110        2.000        0.816  0  0  0  3  2  1
\end{verbatim}
\end{myboxCode}
...
O arquivo \verb|*_variations.csv| a seguir resume esses dados, com a variação sorteada para cada estudante. Observa-se que as variações 19 e 12 com a menor média pela taxonomia de Bloom foram sorteadas para os estudantes da turma \verb|Turma Exemplo|.

\begin{myboxCode}{corCSV}{\textbf{Arquivo \texttt{*\_variations.csv}}}\vspace{3mm}
\hrule
\begin{verbatim}
Room          ID  Name                               Variation  MeanPreviousAbilities
Turma Exemplo 123 João da Silva                          19        -5.0
Turma Exemplo 987 Maria da Silva Andrade de S Gonçalves  12        -5.0
fz-turmaTeste   1 Estudante Exemplo Sobrenome            18        -0.333
\end{verbatim}
\end{myboxCode}
