...
Para ilustrar esse processo, foi criado um exame com 3 QM e 1 QT, todas com dificuldade 1. Além disso, foram selecionadas 6 QM para serem sorteadas, com apenas 3 delas sendo escolhidas com base no desempenho do estudante em exames recentes. Na barra de rolagem, foi escolhido o número 5 para o exame adaptativo, ou seja, será calculada a média das 5 últimas habilidades dos exames realizados pelo estudante. Foram solicitadas a criação de 20 variações de exames antes de clicar no botão ``Criar-Variações''. Também foram selecionadas duas turmas. Após clicar no botão ``Criar-PDF'', os arquivos foram enviados para o email do professor, além de um \textit{download} de um arquivo compactado contendo os seguintes arquivos.
...
\hrule
\begin{verbatim}
Variation  VariationID  MeanAbilities
 4         61096         8
 9         61101         8
12         61104         8
17         61109         8
...
O arquivo \verb|_e486_varID_61093_variations.csv| a seguir resume esses dados, com a variação sorteada para cada estudante. Observa-se que as variações 19 e 17 com a menor média pela taxonomia de Bloom (8) foram sorteadas para os estudantes da turma \verb|Turma Exemplo|.
...
Ao corrigir um exame com questões de múltipla escolha, diversas estatísticas são calculadas, incluindo os parâmetros $a$, $b$ e $c$ da Teoria da Resposta ao Item (TRI). Estes parâmetros representam, respectivamente, a discriminação, a habilidade e o chute do item, conforme detalhado na Seção \ref{sec:questaoQM}. A Figura \ref{fig:cap04_figTRI} ilustra a Curva Característica do Item (CCI) com base nesses três parâmetros.

O código desenvolvido no MCTest v.5.3 tem como objetivo estimar os parâmetros $a$, $b$ e $c$ da TRI para cada questão de múltipla escolha de um exame, utilizando o método da máxima verossimilhança.

Inicialmente, uma função gera um dicionário contendo as respostas de cada aluno para cada questão no formato de 1 para acerto e 0 para erro, onde a chave do discionário é o ID da questão no banco de dados.
...
Estes cálculos estimam os parâmetros do modelo IRT triparamétrico (TRI) para cada questão multiple choice de um exame utilizando máxima verossimilhança.
\
A função de verossimilhança negativa $L(\theta,\alpha,\beta,\gamma|\mathbf{x})$ é definida como:

\begin{align*}
L(\theta,\alpha,\beta,\gamma|\mathbf{x}) &= -\sum_{i=1}^N x_i \log P(X_i=1|\theta_i, \alpha,\beta,\gamma) + (1-x_i)\log[1-P(X_i=1|\theta_i, \alpha,\beta,\gamma)],
\end{align*}
\
onde $P(X_i=1|\theta_i, \alpha,\beta,\gamma) = \gamma + (1-\gamma)F(\alpha(\theta_i - \beta))$ e $F$ é a função logística cumulativa.

A estimação é realizada minimizando $L(\theta,\alpha,\beta,\gamma|\mathbf{x})$ usando o algoritmo \verb|L-BFGS-B|, com restrições $\theta\in[-5,5]$, $\alpha\in(-\infty,+\infty)$, $\beta\in[-5,5]$ e $\gamma\in[0,1]$. Valores iniciais são obtidos dos parâmetros armazenados ou valores fixos.

Após a convergência, os parâmetros estimados $\hat{\alpha}$, $\hat{\beta}$ e $\hat{\gamma}$ são atualizados no banco de dados para cada questão.
