...
Nesta seção, é apresentada uma questão paramétrica que oferece uma ampla variedade de configurações, nos Códigos~\ref{lst:questaoQM_Extra1_parte1},~\ref{lst:questaoQM_Extra1_parte2},~\ref{lst:questaoQM_gerar_QM_itens_parte1},~\ref{lst:questaoQM_gerar_QM_itens_parte2},~\ref{lst:questaoQM_gerar_QM_itens_parte3} e~\ref{lst:questaoQM_gerar_QM_itens_parte4}.
%
...
\begin{listing}[!ht]
    ...
    \begin{minted}[xleftmargin=20pt,linenos=true]{python}
#
# Parte 2 - Definições dos Atributos e chamada do método gerar_QM_itens
#
...
\subsection{Explicação do método \texttt{gerar\_QM\_itens}}\label{sec:metodo_Gerar_QM_itens}
...
Esta seção apresenta o método \texttt{verifica\_possibilidade}, responsável por validar os parâmetros necessários para a geração de variações de questões. O foco principal é a verificação das condições que garantem a formação adequada dos itens, considerando restrições matemáticas e combinatórias.

\begin{description}
    \item[\texttt{verifica\_possibilidade():}] 
    Este método verifica a possibilidade de gerar variações de uma questão com base nos conjuntos de afirmações verdadeiras (\textit{setTrue}) e falsas (\textit{setFalse}), utilizando a função \textit{comb} da biblioteca \texttt{scipy.special}.  
    ...
    \begin{enumerate}
        \item Se a quantidade de itens e afirmações corretas é suficiente:
        \begin{equation}
            N \cdot C \geq A, \quad C < N, \quad P \leq T
        \end{equation}
        \item Se há combinações suficientes para gerar os itens corretos:
        \begin{equation}
            \binom{T}{P} \geq C
        \end{equation}
        \item Se há combinações para formar os itens da questão:
        \begin{equation}
            \binom{T+F}{P} \geq N
        \end{equation}
        \item Se há combinações para gerar todas as alternativas:
        \begin{equation}
            \binom{N}{P} \geq A
        \end{equation}
    \end{enumerate}
    
    Se todas as condições forem atendidas, a questão pode ser gerada. O método retorna a variável lógica \verb|pode_gerar| e mensagens de erro na variável \verb|itens_format| para ajustes nos parâmetros, caso \verb|pode_gerar=False|.  
    
    O número de variações possíveis da questão é dado por:
    \begin{equation}
        \binom{\binom{T+F}{P}}{N}
    \end{equation}
    
    Ver Código~\ref{lst:questaoQM_gerar_QM_itens_parte2}. Mais detalhes podem ser consultados no arquivo \href{https://github.com/fzampirolli/mctest/blob/master/topic/Utils.py}{\texttt{Utils.py}} no GitHub. 
\end{description}
...
\begin{listing}[!ht]
  ...
  \begin{minted}[xleftmargin=20pt,linenos=true]{python}
# ...
# Parte 2 - Definição do Método verifica_possibilidade

# Verifica possibilidade de criar variações
def verifica_possibilidade(setTrue, setFalse, num_itens, \
                           num_afirmacoes_corretas_por_item, 
                           num_itens_corretos, num_alternativas):
  # ... ver código complento no GitHub  
  
  return pode_gerar, itens_format

pode_gerar, itens_format = \
verifica_possibilidade(setTrue, setFalse, num_itens, \
                       num_afirmacoes_corretas_por_item, \
                       num_itens_corretos,\
                       num_alternativas)
...
\end{minted}
\end{myboxCode}
\caption{Método \texttt{gerar\_QM\_itens} -- Parte 2: Método \texttt{verifica\_possibilidade()}.}
...
\subsection{Explicação do método \texttt{gerar\_QM\_itens}: Laço Principal}
