...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.92\textwidth]{ApeA_relato1.png}
...
O gráfico de barras da Figura \ref{fig:ApeA_Acessos_EPs} apresenta o número de alunos que tentaram resolver cada um dos 80 EPs, distribuídos ao longo do eixo \( x \). No eixo \( y \), está representado o número de alunos que tentaram realizar cada EP. Além disso, o gráfico inclui uma linha de tendência exponencial com um coeficiente de determinação \( R^2=0.449 \), indicando que a linha é uma aproximação moderada para os dados. Cada barra no gráfico corresponde a um EP específico, como ``EP1\_1'', ``EP1\_2'', ..., ``EP6\_23''. Observa-se uma variação considerável no número de acessos entre os diferentes EPs, indicando diferentes níveis de interação dos alunos com o material disponibilizado. A linha de tendência exponencial ajuda a visualizar a tendência geral de decrescimento dos acessos ao longo dos EPs. Isso pode indicar o baixo desempenho dos alunos na prova final, como relatado na próxima seção. A cor verde é associada aos EPs relacionados a vetores, enquanto a cor salmão representa os EPs relacionados a matrizes.

\begin{figure}[!ht]
    \centering
    \rotatebox{270}{\includegraphics[width=0.96\textheight]{ApeA_Acessos_EPs.png}}
    \caption{Quantidade de alunos que tentaram resolver os EP (verde para EPs sobre vetores e salmão para matrizes).}
